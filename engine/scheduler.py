...
def main():
    ...
    try:
        while True:
            ...
            # ── Shadow paper-ledger refresh: every 15 min ───
            # Rebuilds logs/shadow_trades.csv from signals_all.csv so the virtual
            # trade log (WIN/LOSS, R, $) stays current. Safe — never touches trading.
            _shadow_sec += 30
            if _shadow_sec >= 900:
                _shadow_sec = 0
