        ...
        try:
            ...
            if mode == "monitor":
                ...
                log_signal(bar_time, signal, result, float(live_ohlc["close"].iloc[-1]),
                           "MONITOR", equity=_cur_equity, trading_equity=_cur_trading_eq)
                write_dashboard(session, core.virtual_trades, tick.bid,
            ...
            if not TEST_MODE and not is_allowed_slot(bar_time):
                ...
                log_signal(bar_time, "SKIP", result, float(live_ohlc["close"].iloc[-1]),
                           "LIVE", equity=_cur_equity, trading_equity=_cur_trading_eq)
                write_dashboard(session, core.virtual_trades, tick.bid,
                                result, _engine_meta(engine))
                time.sleep(POLL_SEC)   # FIX #6: vSL must stay monitored (1s heartbeat)
                continue
            ...
            if signal in ("BUY", "SELL") and core.virtual_trades:
                ...
                if handled:
                    log_signal(bar_time, signal, result, float(live_ohlc["close"].iloc[-1]),
                               "LIVE", lot=0.0, equity=_cur_equity, trading_equity=_cur_trading_eq)
                    write_dashboard(session, core.virtual_trades, tick.bid,
                                    result, _engine_meta(engine))
                    time.sleep(POLL_SEC)   # FIX #6: vSL must stay monitored (1s heartbeat)
                    continue
            ...
            if signal in ("BUY", "SELL") and getattr(CFG.filters, "skip_range_phase_entry", False):
                ...
                if _irp == 1 and (_rmp <= 0 or win_prob < _rmp):
                    ...
                    log_signal(bar_time, "SKIP", result, float(live_ohlc["close"].iloc[-1]), "LIVE", equity=_cur_equity, trading_equity=_cur_trading_eq)
            ...
            if signal in ("BUY", "SELL") and not core.virtual_trades and not _range_block and not _ctf_block and not _pb_block and not _smma_block and not _adx_block:
                ...
                if _resume_pending:
                    ...
                    if not _ask_resume(signal, _px0, win_prob,
                        ...
                        log_signal(bar_time, "SKIP", result, _px0, "LIVE", equity=_cur_equity, trading_equity=_cur_trading_eq)
                        write_dashboard(session, core.virtual_trades, tick.bid, result, _engine_meta(engine))
                        time.sleep(POLL_SEC)
                        continue
                ...
                log_signal(bar_time, signal, result, float(live_ohlc["close"].iloc[-1]),
                           "LIVE", lot=used_lot or 0.0, sl=0.0, tp=0.0, equity=_cur_equity, trading_equity=_cur_trading_eq)
            elif not _range_block and not _ctf_block and not _pb_block:
...
def _pre_pop_dashboard(engine, core, session, news_df):
    ...
    try:
        ...
        res["bar_time"] = str(bt)
        core._last_signal = res
        write_dashboard(session, core.virtual_trades, tick.bid, res, _engine_meta(engine))
