...
# ── Last actionable (BUY/SELL) signal cache ──────────────────────────────────
# 2026-07-09 (Imtiyaz): the dashboard's DECISION AREA (signal box + AI summary + market
# intelligence) must show the LAST BUY/SELL signal with ALL its params — a plain SKIP bar must
# NOT overwrite/blank the box. Persisted to disk so it survives a bridge restart. This affects
# ONLY the display; the pure per-bar signal is still logged every bar to signals_all.csv.
...
def _lts_path():
    try:
        return Path(CFG.paths.logs_dir) / "last_trade_signal.json"
    except Exception:
        return Path("logs") / "last_trade_signal.json"


# Numeric columns in signals_all.csv → coerced to float on bootstrap so the decision block's
# arithmetic (ev_r = wp*tp_m …, round(state_prob,4)) never hits a str × float TypeError.
_LTS_NUM_COLS = (
    "win_prob", "state_prob", "dir_prob", "big_win_prob", "price", "lot", "sl", "tp",
    "atr20_pct", "vol_spike", "in_range_phase", "slot_wr", "h4_bull_ob_dist",
    "h4_bear_ob_dist", "equity", "trading_equity", "move",
)


def _bootstrap_last_trade_signal():
    """Fresh start with NO persisted cache: seed the decision box from the most recent BUY/SELL
    row in signals_all.csv, so the box shows the last real signal immediately instead of sitting
    on SKIP for hours until the next live BUY/SELL. The block computes eff_prob / ev_r / risk_grade
    / market-structure from these raw fields, so a CSV row is enough. Returns a dict or None."""
    try:
        import csv as _csv
        p = Path(CFG.paths.logs_dir) / "signals_all.csv"
        if not p.exists():
            return None
        last = None
...
def _remember_last_trade_signal(sig):
    """Return the signal to DISPLAY in the decision area: the most recent BUY/SELL with all its
    params. A SKIP bar keeps showing the last actionable signal (box never blanks). Survives a
    restart via logs/last_trade_signal.json. On any error, returns `sig` unchanged (old behaviour)."""
    global _LAST_TRADE_SIG, _LTS_LOADED
    try:
        if not _LTS_LOADED:
            _LTS_LOADED = True
            p = _lts_path()
            if p.exists():
                try:
                    _LAST_TRADE_SIG = json.loads(p.read_text(encoding="utf-8"))
                except Exception:
                    _LAST_TRADE_SIG = None
            # No persisted cache yet (first run after this feature shipped) → bootstrap from CSV so
            # the box isn't stuck on SKIP until the next live BUY/SELL.
            if not _LAST_TRADE_SIG:
                _LAST_TRADE_SIG = _bootstrap_last_trade_signal()
...
def write_dashboard(session, virtual_trades, current_price, last_signal=None,
    ...
    try:
        ...
        # ── Signal info ───────────────────────────────────────
        # 2026-07-09 (Imtiyaz): DISPLAY the last actionable (BUY/SELL) signal with ALL its params.
        # Everything below (hmm/state/dir prob, market_structure, eff_prob, ev_r, risk_grade,
        # ai_summary, market_intel) is derived from `sig`, so freezing it here keeps the whole
        # decision block coherent. Live price / session / open-trades / countdown come from `tick`
        # (not `sig`) → they stay live. The pure per-bar signal is still logged every bar.
        _raw_sig = last_signal or {}
        _raw_actionable = isinstance(_raw_sig, dict) and _raw_sig.get("signal") in ("BUY", "SELL")
        sig  = _remember_last_trade_signal(_raw_sig)
        # Showing a CACHED past signal (current bar was SKIP/none) → render it as CONFIRMED (not
        # "forming"), and flag it so the box can show a "🕒 last signal" hint with its own time.
        _sig_cached = (not _raw_actionable) and isinstance(sig, dict) and sig.get("signal") in ("BUY", "SELL")
        if _sig_cached:
