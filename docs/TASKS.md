...
| **Volatile counter-HTF gate — CODE DONE, WFO PENDING** | Signal-audit + Fable-5 second opinion found: honest 53-week WFO baseline shows Volatile-regime trades in the 42-48% win_prob band that go AGAINST dominant HTF direction are net-losing (n=38, -1.9R, PF 0.88) vs the same band aligned WITH HTF (n=48, +18.9R, PF 3.78). Confirmed NOT a time/slot confound (slot_win_rate/hour/day/week all checked, spread evenly). Implemented as `QGAI_VOL_HTF_GATE` env-gated SKIP in `inference.py` (default OFF — zero live impact until explicitly tested/enabled). **⏳ NEXT: `Run_VolHTFGate_AB_WFO_TEST.bat` (5wk) → if B>=A, `Run_VolHTFGate_AB_WFO_FULL.bat` (53wk) → only adopt live if full WFO confirms.** |
