...
| # | Task (કામ) | પરિણામ |
|---|------------|--------|
| **ADX-death exit** | Imtiyaz idea + Fable-5 design: K/4 TF ADX slopes ≤0 for N bars + profit ≥ X×R → exit | Code DONE. **18-cell sweep DONE (2026-07-09):** K{2,3}×N{2,3,4}×X{0.3,0.5,1.0}. Baseline (OFF) +344.7R. **Top-2: K3N4X1.0 +409.1R (+64.4R best), K3N4X0.3 +404.7R (+60.0R).** K=2 all underperform. Results: `backtest/results/adx_death_sweep/`. **WFO validation bats ready** (`Run_ADXDeath_WFO_TEST.bat` + `Run_ADXDeath_WFO_Validate.bat`). |
...
| # | Task | સ્થિતિ |
|---|------|--------|
| **ADX-Death WFO** | WFO validation for top-2 cells (K3N4X1.0 + K3N4X0.3) + baseline | TEST bat ready, FULL bat ready. Gate >= +444.7R. Bat: `Run_ADXDeath_WFO_TEST.bat` then `Run_ADXDeath_WFO_Validate.bat` |
...
| # | Task | વિગત |
|---|------|------|
| **RAW-VOL retrain** (2026-07-09, Imtiyaz) | `tick_volume` raw feature ઉમેર્યું → RETRAIN + WFO-gate | Code DONE: `"tick_volume"` (raw MT5 tick count, no normalization) `features.py` FEATURE_COLS માં ઉમેર્યું (~line 1183); normalized `"volume"` pruned જ રહે. **TEST-bat PASSED (2026-07-09):** `Start/3_Train_Models_TEST.bat` → 5/5 checks ✓ (36 features, tick_volume idx 25, matrix built 120×36 no crash, values RAW 1554–8957 range, mean 5175 — clearly not the old 0–5 ratio). No model written, live `.pkl` untouched. **NOT LIVE — live .pkl માં column નથી → feature-mismatch.** **Retrain DONE (2026-07-09 12:45-49):** combined AUC 0.6731 (n=2743), BUY AUC 0.6858 (n=1561), SELL AUC 0.6825 (n=1182), all 36 feat incl. `tick_volume` (importance rank #17/37 = 0.0283, real signal not noise), single consistent run verified. ⚠️ **Bug found + fixed:** `Start/3_Backup_Models_BEFORE_Retrain.bat` was run AFTER retrain (file mtimes: models written 12:45-49, backup folder 12:55) → `models_20260709_125558/` backed up the NEW model, not the old baseline — true pre-tick_volume baseline was briefly unrecoverable via the official backup path. **Fixed:** recovered the true pre-retrain model (AUC 0.6734, no tick_volume, timestamps 11:05-11:09) from `audit/feature_audit_20260709/workspace/data/models/final/` → copied to `data/models/backups/models_PRE_TICKVOL_20260709_1105/final/`, verified no `tick_volume` in its `model_meta.json`. **Lesson: run the backup bat BEFORE `3_Train_Models.bat`, not after — confirm the backed-up meta.json predates the retrain timestamp before trusting it.** NEXT: deep bug-check (parity/feature-mismatch check on the retrained model) → **WFO-gate ≥ current baseline R (+444.7R)** પહેલાં live adopt. raw volume noise ઉમેરી શકે; model importance≈0 કાઢે એ પણ માન્ય પરિણામ. REVERT: FEATURE_COLS માંથી `"tick_volume"` line કાઢો + `models_PRE_TICKVOL_20260709_1105` restore. |
