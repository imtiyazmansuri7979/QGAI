...
| **67-feature validation sweep — 3-stage plan (Imtiyaz, detailed spec 2026-07-13)** | `engine/run_feature_sweep.py` redesigned around Imtiyaz's exact priority plan: (1) **priority_batch** (10 features, run first — `Run_FeatureSweep_PriorityBatch.bat`): `h4_support_dist, h1_resist_dist, move_2hr, ts_line_dist_pct, tick_volume` (dropped, unprune-tested) + `H4_DI_diff, h4_adx_slope, move_4hr, momentum_aligned_2hr` (already active, ablate-tested) + `h1_support_dist`. (2) **Stage 1 = active** (27, `Run_FeatureSweep_Tier1_Active.bat`, importance-ranked — top ~20-24 already covers all of Imtiyaz's flagged active features). (3) **Stage 2 = high_prob** (SMMA-trend family + raw ADX + OB/SR + regime composites, `Run_FeatureSweep_Tier2_HighProbability.bat`). (4) **Stage 3 = remaining** (`Run_FeatureSweep_Tier3_Remaining.bat`). Each feature auto-routed to ablate (if active) or unprune (if dropped) by checking `features.py` at run time — a tier list can freely mix both. Every test now also computes BUY/SELL split, Ranging/Trending/Volatile split, and week-by-week consistency from the real trades CSV (not just the summary report), plus an automated verdict (`CORE_KEEP` / `DROP_CANDIDATE` / `NEEDS_1YEAR_CONFIRMATION` / `CONFIRMED_DROPPED` / `NEUTRAL_REDUNDANT` / `REVIEW`) per Imtiyaz's detailed pass/fail checklist — verdicts other than the two "confirmed" ones still need Stage 2 (1-year) + Stage 3 (WFO) before trusting, per the project's own stage-gate. Note: `m15_adx_slope` (mentioned in the spec) does not exist in the codebase — excluded, only `h4_adx_slope`/`h1_adx_slope` exist. **⏳ NEXT: run `Run_FeatureSweep_PriorityBatch.bat` first (tonight), then Tier1→2→3 on subsequent nights.** |
