...
### S. HIGH - Wilder ADX smoothing used for rolling ADX slopes/switch, inconsistent with the always-EMA adx_merged.csv and the live EA's iADX() call
**Files:** `engine/features.py` (`_wilder_adx`), `engine/fresh_reload.py`, `engine/research_smma_adx_score_sweep.py`, `engine/research_smma_adx_snapshot_logger.py`  
**Reported:** 2026-07-17 by Imtiyaz ("you use wilder adx for all calculation it wong").  
**Evidence:** `mt5_data_updater.py`/`regen_adx_asof.py`/`regen_adx_di.py` (feeding `adx_merged.csv` -> `M15_ADX`/`H1_ADX`/`H4_ADX`/`*_DI_diff`) always used `ewm(span=14, adjust=False)` (alpha=2/15). But `features.py:_wilder_adx()` (feeding `h4_adx_roll`/`h1_adx_roll`/`h4_adx_slope`/`h1_adx_slope`/`ts_adx_switch_trend` via `build_indicators.py` -> `indicators_merged.csv`) used Wilder recursion (alpha=1/14, ~1.9x slower). Ground truth confirmed via the live MT5 EA source (`Trend Signal Pro EA.mq5`, pasted by Imtiyaz, not in this repo): `handle_ADX = iADX(_Symbol,ADX_TF,ADX_Period);` — the live system calls MT5's standard `iADX()`, not `iADXWilder()` (MT5 exposes both as separate functions). `fresh_reload.py` additionally had a hidden landmine: its docstring claimed "same approach as mt5_data_updater" while actually running Wilder math.  
**Scope correction:** NOT "all data/features/models used Wilder" — only the rolling/slope/switch features were Wilder-scale; `adx_merged.csv` columns were always EMA and unaffected.  
**Root cause:** two independently-written ADX implementations in the codebase (one EMA, one Wilder) that were never cross-checked against each other or against the live EA's actual `iADX()` call.  
**Fix:** `_wilder_adx()` -> `_ema_adx()` (identical `ewm(span=14)` formula as the always-correct path); same fix applied to `fresh_reload.py` and the two research scripts. `indicators_merged.csv` regenerated + `build_indicators.py --verify 100` PASSED. Model retrained (`model_created_at` 2026-07-17T20:15:55; `feature_importance.csv` confirms sane non-zero importances: `H4_ADX=0.0433`, `h4_adx_slope=0.0388`, `h1_adx_slope=0.0384`).  
**Verification:** `python -m py_compile` on all 4 changed files passed. Full enterprise-style archive + migration report (backup, Wilder-era archive with SHA-256 hashes, audit, final report) consolidated under `C:\OLD_QGAI\` — see `C:\OLD_QGAI\QGAI_MIGRATION\WILDER_TO_EMA_MIGRATION_REPORT.md`. `C:\QGAI` itself untouched; new MT5-parity diagnostic tools live in `engine/`, registry at `backtest/_runners/adx_ema_migration/`.  
**Remaining:** Test 1 (live MT5 `iADX()` numeric parity) toolkit built but requires Imtiyaz to run the MT5-terminal half manually (`backtest/_runners/adx_ema_migration/ADXMIG-01_RUN_MT5ParityTest.bat`). No post-fix backtest run yet — required before any keep/drop decision per `docs/BACKTEST_RESULT_AUDIT.md`. Two independent EMA ADX implementations still exist (`features.py` vs `bridge_main.py`->`regen_adx_asof.py`) — same architectural pattern that caused this bug; flagged for future consolidation, not fixed here.
