...
**Verification:** `python -m py_compile` on all 4 changed files passed. Full enterprise-style archive + migration report: `C:\QGAI_MIGRATION\WILDER_TO_EMA_MIGRATION_REPORT.md`, audit: `C:\QGAI_MIGRATION\audit\WILDER_TO_EMA_ADX_AUDIT.md`, Wilder-era archive: `C:\QGAI_ARCHIVE\ADX_WILDER\WILDER-REG-001\` (read-only).  
**Remaining:** Test 1 (live MT5 `iADX()` numeric parity) toolkit built but requires Imtiyaz to run the MT5-terminal half manually (`C:\QGAI_EMA_ADX\11_runners\EADX-002_RUN_MT5ParityTest_Step1_PythonExport.bat`). No post-fix backtest run yet — required before any keep/drop decision per `docs/BACKTEST_RESULT_AUDIT.md`. Two independent EMA ADX implementations still exist (`features.py` vs `bridge_main.py`->`regen_adx_asof.py`) — same architectural pattern that caused this bug; flagged for future consolidation, not fixed here.
