...
- **ESTIMATED TIME in every bat banner (Imtiyaz, 2026-07-17): every backtest/WFO/sweep bat file MUST include an `Estimated time:` line in its banner section** — computed from the timing benchmarks in `backtest/_runners/feature_sweep_67/README.md` § "Backtest Timing Reference". Formula: `arms × per-arm-time`. Per-arm benchmarks: TEST 2-week ~2.5 min, 3-month ~20 min, H2 6-month ~27 min, 1-year ~60 min, WFO 26-week ~3h. Analysis-only (no train) ~5-10 min. This tells the operator upfront how long to wait before checking back.
