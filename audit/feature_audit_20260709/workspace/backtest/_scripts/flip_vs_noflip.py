"""H1 flip-exit ON vs OFF. H1 stop+trail, buf 0.20, cost $0.30, H1+H4 aligned, no TP.
'OFF' = trade exits ONLY when the trailing H1-line stop is hit (no exit on H1 trend flip).
Shows return + maxDD + Monte-Carlo P(DD>50%) at 3% risk, to decide if removing flip helps."""
...
def run(use_flip):
    ...
    while i<n-1:
        ...
        while k<n:
            ...
            if use_flip and (not np.isnan(t1[k]) and int(t1[k])==-d): ex=C[k]; break
...
def metrics(R):
    ...
    return len(R),win,R.mean(),R.sum(),pf,eq,dd,(mdd>50).mean()*100,mc
print("="*100)
print(f"H1 FLIP-EXIT  ON vs OFF | H1 stop+trail, buf {BUF}, cost ${COST}, H1+H4 aligned, no TP, {RISK}% risk")
print("="*100)
print(f"{'mode':>22}{'trades':>8}{'win%':>7}{'avgR':>8}{'totalR':>8}{'PF':>6}{'$10k->':>11}{'maxDD':>8}{'MC P(DD>50%)':>14}{'maxConsLoss':>12}")
for lbl,uf in [("FLIP ON (current)",True),("FLIP OFF (stop only)",False)]:
    nt,win,av,tot,pf,eq,dd,pdd,mc=metrics(run(uf))
    print(f"{lbl:>22}{nt:8d}{win:7.1f}{av:+8.3f}{tot:+8.1f}{pf:6.2f}{eq:>11,.0f}{dd:>7.0f}%{pdd:>13.0f}%{mc:>12d}")
print("\nIf FLIP OFF gives lower maxDD/P(DD>50%) with similar return -> remove flip. Else keep it.")
