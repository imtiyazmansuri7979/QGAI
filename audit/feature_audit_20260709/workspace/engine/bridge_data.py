...
# 2026-07-09: ensure the signal CSV + SQLite have a trailing `trade_action` column
...
def log_signal(bar_time, signal, result, price, mode, lot=0.0, sl=0.0, tp=0.0,
    ...
    `signal` = PURE ENGINE decision (BUY/SELL/SKIP by threshold) — backtest-identical,
    logged on EVERY bar regardless of whether MT5 places a trade (2026-07-09 DECOUPLE).
    `trade_action` = what the RECEIVER (MT5) did with that signal, kept SEPARATE from the
    signal column: EXECUTED / EXEC_FAILED / HOLD_IN_TRADE / OPPOSITE_HANDLED / MONITOR /
    BLOCK_SLOT / BLOCK_RANGE / BLOCK_CTF / BLOCK_PULLBACK / BLOCK_SMMA / BLOCK_ADX /
    RESUME_SKIP / NO_TRADE. System = signal PROVIDER, MT5 = signal RECEIVER.
    ...
    # Auto-append to signals_complete.csv (keeps it current without manual rebuild)
    try:
...
def write_outcome(entry_orders_today, closed_deals):
    ...
    # signals_complete.csv outcome backfill (update last N rows only — fast)
    try:
        _sc_path = os.path.join(os.path.dirname(str(CFG.paths.signal_log)), "signals_complete.csv")
        if os.path.exists(_sc_path) and outcome_map:
            _sc_rows = []
            _sc_updated = 0
            with open(_sc_path, "r", newline="", encoding="utf-8") as f:
                reader = csv.DictReader(f)
                _sc_fnames = list(reader.fieldnames or [])
                if "outcome" not in _sc_fnames:
                    _sc_fnames.append("outcome")
                if "move" not in _sc_fnames:
                    _sc_fnames.append("move")
                for row in reader:
                    bt = row.get("bar_time", "")
                    if (row.get("outcome", "") in ("", "PENDING") and
                            row.get("mode", "") in ("LIVE", "MONITOR") and
                            bt in outcome_map):
                        row["outcome"] = outcome_map[bt][0]
                        row["move"]    = outcome_map[bt][2]
                        _sc_updated += 1
                    _sc_rows.append(row)
            if _sc_updated:
                _sc_tmp = _sc_path + ".tmp"
                with open(_sc_tmp, "w", newline="", encoding="utf-8") as f:
                    writer = csv.DictWriter(f, fieldnames=_sc_fnames)
                    writer.writeheader()
                    writer.writerows(_sc_rows)
                shutil.move(_sc_tmp, _sc_path)
    except Exception:
        pass
