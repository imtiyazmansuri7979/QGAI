...
def main():
    ...
    # Append HMM state as feature unless this isolated audit run ablates it.
    _ablate_set = {x.strip() for x in _os.environ.get("QGAI_ABLATE", "").split(",") if x.strip()}
    if "hmm_state" in _ablate_set:
        print("  [AUDIT ABLATE] hmm_state removed from training feature matrix")
        X_tr_full, X_va_full, X_te_full = X_tr, X_va, X_te
...
def train_directional_models():
    ...
    _ablate_set = {x.strip() for x in _os.environ.get("QGAI_ABLATE", "").split(",") if x.strip()}
    feat_full = feat_names if "hmm_state" in _ablate_set else feat_names + ["hmm_state"]
    if "hmm_state" in _ablate_set:
        print("  [AUDIT ABLATE] hmm_state removed from BUY/SELL feature matrices")
    ...
    for label, mask in [("BUY", buy_mask), ("SELL", sell_mask)]:
        ...
        if "hmm_state" in _ablate_set:
            X_tr, y_tr = X_dir[:tr_end], y_dir[:tr_end]
            X_va, y_va = X_dir[tr_end:va_end], y_dir[tr_end:va_end]
            X_te, y_te = X_dir[va_end:], y_dir[va_end:]
        else:
